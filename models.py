...
class KGReasoning(nn.Module):
    ...
    '''
    def embed_query(self, queries, idx=0, use_cuda=False):
        
        #embed queries based on the geometry (geo) mode.
        
        if self.geo == 'beta':
            return self.embed_query_beta(queries, ('e', ('r',)), idx)  # only 1p queries for now
        elif self.geo == 'box':
            return self.embed_query_box(queries, ('e', ('r',)), idx)
        elif self.geo == 'vec':
            return self.embed_query_vec(queries, ('e', ('r',)), idx)
        else:
            raise ValueError("Unknown geo type: {}".format(self.geo))
    '''
    def cal_logit_beta(self, entity_embedding, query_dist):
        ...
        return logit
    '''
    def compute_logit(self, query_embedding, answer):
        
        #compute score between query embedding and answer embed.
        
        if self.geo == 'beta':
            answer_embedding = self.entity_regularizer(torch.index_select(self.entity_embedding, dim=0, index=answer))
            answer_embedding = answer_embedding.unsqueeze(1)  # shape (batch_size, 1, 2 * dim)
            query_alpha, query_beta = torch.chunk(query_embedding, 2, dim=-1)
            query_dist = torch.distributions.beta.Beta(query_alpha, query_beta)
            logit = self.gamma - torch.norm(torch.distributions.kl.kl_divergence(query_dist, torch.distributions.beta.Beta(answer_embedding[..., :query_alpha.size(-1)], answer_embedding[..., query_alpha.size(-1):])), p=1, dim=-1)
        else:
            answer_embedding = torch.index_select(self.entity_embedding, dim=0, index=answer)
            logit = self.gamma - torch.norm(answer_embedding - query_embedding, p=1, dim=-1)
        return logit
    '''
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        for query_structure in batch_queries_dict:
            batch_queries_dict[query_structure] = torch.LongTensor(batch_queries_dict[query_structure]).cuda() if args.cuda else torch.LongTensor(batch_queries_dict[query_structure])
        if args.cuda:
        ...
        #generate corrupted queries
        batch_queries_dict_corrupted = {}
        for query_structure, queries in batch_queries_dict.items():
            corrupted = queries.clone()
            #corrupt the anchors for all querries
            corrupted[:, 0] = torch.randint(0, model.nentity, (queries.size(0),), device=queries.device)
            batch_queries_dict_corrupted[query_structure] = corrupted

        #call model on corrupted queries wit same samples
        positive_logit_corrupted, negative_logit_corrupted, _, _ = model(
            positive_sample, negative_sample, subsampling_weight,
            batch_queries_dict_corrupted, batch_idxs_dict
        )
        #losses
        positive_score = F.logsigmoid(positive_logit).squeeze(dim=1)
        negative_score = F.logsigmoid(-negative_logit).mean(dim=1)
        negative_score_corrupted = F.logsigmoid(-negative_logit_corrupted).mean(dim=1)
        positive_score_corrupted = F.logsigmoid(-positive_logit_corrupted).squeeze(dim=1)  

        pos_loss = - (subsampling_weight * positive_score).sum() / subsampling_weight.sum()
        neg_loss = - (subsampling_weight * negative_score).sum() / subsampling_weight.sum()
        corrupted_neg_loss = - (subsampling_weight * negative_score_corrupted).sum() / subsampling_weight.sum()
        corrupted_pos_loss = - (subsampling_weight * positive_score_corrupted).sum() / subsampling_weight.sum()

        loss = (
            pos_loss * 0.4 +
            neg_loss * 0.4 +
            corrupted_pos_loss * 0.1 +
            corrupted_neg_loss * 0.1
        )
        ...
        return {
            ...
            'corrupted_pos_loss': corrupted_pos_loss.item(),
            'corrupted_neg_loss': corrupted_neg_loss.item(),
        }
